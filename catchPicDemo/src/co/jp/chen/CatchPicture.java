...
public class CatchPicture {
	...
	public static void main(String[] args) {
		...
		String url = "http://zhidao.baidu.com/daily/view?id=13803";
		String charset = "gbk";
		List<Picture> list = new CatchPicture().lookWeiboPic(url, charset,
				regular, "2,1");
		System.out.println(list.size());
	}

	// 根据URL查看网站上的图片
	public List<Picture> lookWeiboPic(String url, String charset,
			String regular, String attIndex) {
		...
		try {
			...
			if (!Util.isEmptyString(url)) {
				...
				if (!Util.isEmptyString(htmls)) {
					...
					// 得到参数属性顺序
					String[] sort = regular.trim().split(","); // 下标：0 表示
																// 标题title ， 1
																// 表示 图片路径
					// 判断后缀后 得到网站的请求头部
					// http://www.moonbasa.com/p-032111106.html-->得到
					...
					while (matcher.find()) {
						...
						if (-1 == Integer.parseInt(sort[1])) {
							...
							String webImgUrl = matcher.group(Integer
									.parseInt(sort[1]));
							// 判断是绝对路径还是相对路径
						...
						String upPath = upload(picture.getSource(),
								"/Users/ChenRukun/Downloads/");
